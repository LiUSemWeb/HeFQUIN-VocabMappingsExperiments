...
do
  ...
  do
    ...
    java -Xmx46048m -cp $engine se.liu.ida.hefquin.cli.RunQueryWithoutSrcSel --query ${QUERY_FILE} --federationDescription $federation --suppressResultPrintout --printLogicalPlan --printQueryProcStats --printQueryProcMeasurements --printPhysicalPlan > $log_file_path 2>&1
  done 
done
